...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function checkUrl(url) {
    try {
      ...
      if (data.status === 'success') {
        ...
        return { url, status: 'invalid', statusCode: data.statusCode || 'unknown', message: data.message || 'Unknown error' };
      }
    } catch (error) {
  ...
  async function validateUrls() {
    ...
    validationResultsContainer.innerHTML = '<div class="p-4 bg-surface rounded-github mb-4">Validating URLs. This may take a while...</div>';
