...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typeof contributionHeatmap === 'undefined') {
    console.error('Contribution data not found. Make sure contributionHeatmap is defined.');
    return;
  }
...
function addDayLabels(svg, config) {
  ...
  dayLabels.forEach((day, index) => {
    if (day) {
      ...
      text.setAttribute('y', 20 + (config.cellSize + config.cellSpacing) * index + config.cellSize / 2 + 4);
...
function addLegend(svg, config, totalWidth, totalHeight) {
  ...
  legendGroup.setAttribute('transform', `translate(${totalWidth - 250}, ${totalHeight - 20})`);
