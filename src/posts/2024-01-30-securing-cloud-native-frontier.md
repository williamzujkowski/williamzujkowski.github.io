...
---
Recently in my homelab, I decided to harden the security of my K3s cluster running on Proxmox. I felt confident. I had firewalls configured, network segmentation via VLANs, and proper TLS certificates everywhere.
...
The wake-up call got worse when I accidentally exposed my Docker socket to other VMs on my Proxmox host while troubleshooting a networking issue. For about 20 minutes, my entire container runtime was accessible to every virtual machine on the hypervisor before I caught the mistake.
...
My first microservices deployment to my homelab seemed secure by traditional standards. I had firewalls, encrypted connections, and proper authentication. What I missed was the exponential growth in attack surface that comes with distributed systems.
...
When I ran OSV-Scanner v1.5.0 across my entire stack recently, I found security issues in places I never expected. Out of 23 services, 18 had at least one transitive dependency with known vulnerabilities. The static security models that worked for monolithic applications typically crumble when faced with dozens of interconnected, ephemeral services.
...
**Complex Interdependencies:** A single compromised service can cascade through the entire system if network policies aren't properly configured. When I misconfigured my Wazuh logging agent, it had access to every container's logs across all namespaces.
...
**Image Security:** Early in my containerization journey, I pulled base images from Docker Hub without verification. When I finally ran Grype v0.74.1 against them, I found that roughly 40% of my 35 container images contained critical vulnerabilities (CVSS 9.0+).

One Alpine 3.18 image had 12 high-severity CVEs. For example, CVE-2023-5678 (a publicly disclosed vulnerability) in the base layer affected every service I built on top of it. Now I only use verified base images and scan everything before deployment.
...
**Registry Security:** Recently, I accidentally pushed a test container image containing a hardcoded API token to my local registry. The token was for a development service, but it taught me a crucial lesson.
...
**Least Privilege Principle:** Every container gets only the minimum permissions required. I learned this when testing container escape techniques on my isolated test node. For more context, see [writing secure code: a developer's guide to thwarting security exploits](/posts/2024-01-08-writing-secure-code-developers-guide).
...
**Network Segmentation:** Implementing proper Kubernetes NetworkPolicies in my K3s v1.28 cluster prevented what could have been a serious issue. I had a misconfigured service that was attempting to connect to my Bitwarden database.
...
**Runtime Monitoring:** I installed Wazuh 4.7.0 for runtime monitoring in my homelab. Within the first week, it detected unusual network connections from a container I was testing.
...
**Authentication and Authorization:** No open APIs, ever. I learned this when I left a metrics endpoint unauthenticated for "just a few days" while debugging.
...
**API Gateways:** I deployed Traefik v2.11 as my API gateway in my homelab. Centralizing API management helped me configure consistent security policies and rate limiting across all 15+ services.
...
**Input Validation:** Serverless functions are still susceptible to traditional attacks like SQL injection and XSS. I learned this when testing a function that accepted user input without validation. The ephemeral nature doesn't excuse proper input validation. The same security principles apply, they just happen in a different execution model.

**Dependency Management:** Vulnerable dependencies in serverless functions can be exploited just like in traditional applications. When I scanned my OpenFaaS functions with Grype v0.74.1, I found that 4 out of 7 functions had vulnerable dependencies. Regular scanning and updates remain essential, probably more so because function dependencies aren't always visible in standard container scans.
...
**Training Investment:** I spent roughly four months learning container security, K3s hardening, and cloud security best practices. I watched probably 30+ hours of conference talks, read CIS benchmarks cover to cover, and practiced container escapes in isolated environments. The investment in education was essential. Without understanding the underlying attack vectors, I would have just been following checklists without comprehending why.
...
**Incremental Approach:** I didn't transform my security posture overnight. It was a gradual process over roughly five months. First came container scanning, then network policies, then RBAC hardening, then runtime monitoring. Each improvement built on the previous one. Trying to add everything simultaneously would probably have resulted in misconfigurations and gaps.

## What I Wish I'd Known Earlier

Looking back on my cloud-native security journey, several insights stand out:

**Start with Basics:** Advanced security features are worthless if you haven't mastered the fundamentals of least privilege, network segmentation, and proper authentication. I wasted probably three weeks setting up a complex service mesh for mutual TLS before I had basic network policies in place. Starting with the fundamentals would have saved time and prevented gaps.

**Security by Design:** Retrofitting security into existing cloud-native applications is exponentially more difficult than building it in from the start. When I tried to add NetworkPolicies to my running K3s cluster, I had to carefully map every service dependency. It took roughly 40 hours. If I had designed with network isolation from the beginning, it probably would have taken 5-10 hours total.
...
**Incident Response Planning:** Traditional incident response plans don't work well for distributed systems. When I had the Docker socket exposure incident (exposed internally to other VMs on my Proxmox hypervisor), my first instinct was to isolate the host. In a cloud-native environment, that would have killed dozens of unrelated services. I had to completely rethink response procedures around pod isolation, namespace quarantine, and service-level incident containment.

## Conclusion

Cloud-native architectures unlock unprecedented agility and scalability, but they demand an equally agile and comprehensive security strategy. The lessons I learned from roughly five months of deployments, incidents, and improvements have shown me that security in distributed systems is both more complex and more critical than traditional applications.
...
My journey from finding 47 high-severity vulnerabilities to building comprehensive security controls taught me humility. Each new deployment is an opportunity to apply lessons learned and strengthen defenses against an ever-evolving threat landscape. The Docker socket exposure incident (exposed internally to other VMs on my Proxmox hypervisor, not externally) now serves as a reminder of how far I've come and how much further I have to go. Security is never "done." It's an ongoing process of learning, adapting, and improving.
