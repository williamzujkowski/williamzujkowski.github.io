...
## "But some of it was pseudocode"

Fair challenge, and worth answering with counts rather than assertion, because
"nobody ran it" is an unfair charge against a snippet that never claimed to be
runnable.

Across the pre-audit corpus: **11 code blocks are self-labelled pseudocode,
simplified, or illustrative. 55 artifacts are introduced as "Complete" or
"Full".** The four in this post are all in the second group — a threat
exporter headed `# Custom exporter: threat_intel.py`, a "**Complete** Grype
configuration", a "**Full** GitHub Actions workflow (109 lines)" described as
"**Complete implementation**". Those are claims of runnability, and they are
the ones the charge applies to.

But the labelled blocks turned out to be more interesting than the exemption I
was about to grant them. Here is one, honestly marked:
...
The code is pseudocode and says so. The **comments are factual claims about an
experiment** — an episode count, a frame rate, a wall-clock duration, a specific
GPU. None of it happened; the whole passage was generated by a pass whose commit
message lists the measurements it added.

So the label did its job on the code and provided cover for everything else in
the block. "This isn't real code" reads, at a glance, as "nothing here is
load-bearing," and the numbers ride through on that. A reader who correctly
skips pseudocode has just skipped a fabricated 72-hour training run.

Which sharpens the recommendation rather than weakening it. **Execute what claims
to be executable; verify what claims to be measured.** The pseudocode exemption
applies to syntax, not to the assertions parked inside it — and a comment saying
you ran something for 72 hours is not pseudocode in any sense.
...
|---|---|---|
| **Execute the artifact once** | works, dominates everything else | every defect died on first run — `NameError` on import, invented TOML rejected by the parser, the gate revealing itself the moment an upstream job fails. Applies to the 55 artifacts introduced as "Complete" or "Full"; see the pseudocode section for the other 11 |
