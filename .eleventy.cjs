...
module.exports = function(eleventyConfig) {
  ...
  eleventyConfig.addCollection("posts", function(collectionApi) {
    ...
    if (filteredPosts.length > 0) {
      console.log(`Latest post: ${filteredPosts[0].data.title} (${filteredPosts[0].date.toISOString()})`);
    } else {
  ...
  const currentReadingData = loadDataFile(path.join(dataDir, 'current-reading.json'), []);
