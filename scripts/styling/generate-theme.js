...
const defaultTheme = {
  ...
  description: params.description || "Dark theme inspired by GitHub's dark mode with OKLCH colors for better color accuracy and accessibility",
  ...
  colors: {
    ...
    accentLightness: params['accent-lightness'] || 0.70,
  ...
  shadows: {
    ...
    elevated: params['shadow-elevated'] || "0 4px 6px -1px rgba(0, 0, 0, 0.2), 0 2px 4px -2px rgba(0, 0, 0, 0.1)"
  },
  borders: {
...
const themePath = params.output || path.join(__dirname, '..', 'src', '_data', 'config', 'theme.json');
