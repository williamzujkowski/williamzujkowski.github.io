...
const IMAGE_CONFIG_PATH = path.join(
  rootDir,
  "src",
  "_data",
  "config",
...
async function processPost(filename, options = {}) {
  try {
    ...
    const description =
      existingMeta.description || generateDescription(title, cleanedContent);
...
async function batchProcessPosts(options = {}) {
  try {
    ...
    const postFiles = files.filter((file) => {
      ...
      const isDirectory =
        existsSync(filePath) && (file === "processed" || file.startsWith("."));
...
async function writePosts(results) {
  ...
  for (const { file, processed } of results) {
    try {
      ...
      await fs.rename(path.join(SOURCE_DIR, file), path.join(PROCESSED_DIR, file));
    } catch (error) {
