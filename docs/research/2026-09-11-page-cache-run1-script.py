...
def run(output, repetitions=60, seed=20260911):
    ...
    with Path(output).open("x") as evidence:
        try:
            with tempfile.TemporaryDirectory(prefix="page-cache-lab-") as directory:
...
def main():
    ...
    if args.command == "worker":
        ...
        run(args.output, args.repetitions, args.seed)
