...
function loadRecentBlogPosts() {
  // Load recent posts from blog_list.json using an absolute path
  fetch("/blog_list.json")
    .then(response => response.json())
    .then(data => {
      // Sort by date (newest first) and take the first 3 posts
      ...
      if (listDiv) {
        recent.forEach(post => {
          ...
          // Each blog post file is located at /blog/{slug}.html
