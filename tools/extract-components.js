...
async function extractComponents() {
  try {
    console.log(`${DRY_RUN ? '[DRY RUN] ' : ''}Extracting components from styles.css...`);
    ...
    for (const component of COMPONENTS) {
      ...
      if (!DRY_RUN) {
        ...
        console.log(`[DRY RUN] Would write to ${componentPath}:\n${componentContent.substring(0, 100)}...`);
      }
    }
    ...
    const updatedStylesContent = 
      ...
      importStatements + 
      '\n\n' + 
      newStylesContent.replace(/\/\* Tailwind directives \*\/[\s\S]*?@tailwind utilities;[\s\S]*?\n\n/, '');
    
    if (!DRY_RUN) {
      await fs.writeFile(STYLES_PATH, updatedStylesContent);
      console.log('Updated styles.css with component imports');
    } else {
      console.log(`[DRY RUN] Would update styles.css with imports:\n${importStatements}`);
    }
