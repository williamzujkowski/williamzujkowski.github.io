            ...
            if (itemSchema.types) {
              for (const [field, expectedType] of Object.entries(itemSchema.types)) {
                if (item[field] !== undefined && !validateType(item[field], expectedType)) {
                  errors.push(`Field ${arrayField}[${i}].${field} should be of type ${expectedType}, got ${typeof item[field]}`);
                }
              }
            }
          }
        ...
        if (typeof data[nestedField] === 'object' && data[nestedField] !== null) {
          ...
          if (nestedSchema.types) {
            for (const [field, expectedType] of Object.entries(nestedSchema.types)) {
              if (data[nestedField][field] !== undefined && !validateType(data[nestedField][field], expectedType)) {
                errors.push(`Field ${nestedField}.${field} should be of type ${expectedType}, got ${typeof data[nestedField][field]}`);
              }
            }
          }
        }
    ...
    for (const field of schema.required) {
      if (data[field] === undefined) {
        ...
        if (schema.types && schema.types[field]) {
          ...
          if (type === 'string') {
            data[field] = field === 'title' ? 'William Zujkowski' : 
                        field === 'description' ? 'Personal website and blog' :
                        field === 'url' ? 'https://williamzujkowski.github.io' :
                        field === 'author' ? 'William Zujkowski' : '';
          } else if (type === 'number') {
            data[field] = 0;
    ...
    for (const [field, expectedType] of Object.entries(schema.types)) {
      if (data[field] !== undefined && !validateType(data[field], expectedType)) {
        try {
          ...
          console.error(`Could not convert ${field} to ${expectedType}: ${error.message}`);
        }
      }
    }
  }
...
async function validateConfigs() {
  console.log(`Validating configuration files${FIX_MODE ? ' (with auto-fix enabled)' : ''}...`);
